...
class CustomLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        for img_no in range(bs):
            ...
            pred_o = pred_tensor[..., 4]  # Objectness score
            pred_c = pred_tensor[..., 5:]  # Class predictions
            pred_xywh = pred_tensor[..., :4]  # Bounding box predictions

            # Extract components from the target tensor
            target_o = target_tensor[..., 4]  # Objectness score
            target_c = target_tensor[..., 5:]  # Class labels
            target_xywh = target_tensor[..., :4]  # Bounding box coordinates
            ...
            box_ious = [torch.diagonal(box_iou(box_convert(target_xywh, 'cxcywh', 'xyxy'),
                                               box_convert(pred_xywh_anchors[anc_id], 'cxcywh', 'xyxy') )) for anc_id in range(self.na)]
            ...
            # Calculate CIOU Loss for bounding box regression where target_p = 1
            ...
            # # Categorical Cross-Entropy Loss for class predictions where target_p = 1
